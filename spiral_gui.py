...
class SpiralGUI:
    ...
    def _build_ui(self):
        ...
        num_frame1.columnconfigure((1, 3, 5, 7), weight=1)
        ...
        ttk.Label(num_frame1, text="Шаг (прям.):").grid(row=0, column=6, padx=(0, 2))
        self.step_var = tk.IntVar(value=10)
        self.step_spin = ttk.Spinbox(
            num_frame1, from_=2, to=100, textvariable=self.step_var, width=8
        )
        self.step_spin.grid(row=0, column=7)
    ...
    def _on_type_change(self, event=None):
        """Включает/выключает поле шага в зависимости от типа спирали."""
        if self.spiral_type.get() == "rectangular":
            self.step_spin.config(state="normal")
        else:
            self.step_spin.config(state="disabled")
    ...
    def _generate(self):
        ...
        try:
            ...
            if spiral_type == "archimedean":
                ...
                spiral_points = generate_rectangular_spiral(w, h, step)
            else:
