...
# Создаём маленькое изображение 100x100 с серым квадратом
gray = np.zeros((100, 100), dtype=np.float32)
gray[30:70, 30:70] = 128
...
for row in range(int(h / dy) + 3):
    for col in range(int(w / dx) + 3):
        ...
        if lines_in_cell > 0:
            print(f"Cell ({col},{row}) center=({cx:.0f},{cy:.0f}) brightness={avg_brightness:.0f} step={step:.1f} lines={lines_in_cell}")

print(f"\nTotal cells: {total_cells}, Total lines: {total_lines}")
img.save('test_hex4.png', quality=95)
print('Saved test_hex4.png')
